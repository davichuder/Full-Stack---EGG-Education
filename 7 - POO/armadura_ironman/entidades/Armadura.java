...
public class Armadura {
    ...
    public final String[] nombres_objetivos = { "barco", "avion", "auto", "alien", "thanos", "robot", "gangster",
            "perro", "gato", "monstruo" };
    ...
    public void radar() {
        for (int i = 0; i < this.datos_objetivos.length; i++) {
            this.distancias_objetivos[i] = Math.sqrt(Math.pow(this.datos_objetivos[i][3], 2)
                    + Math.pow(this.datos_objetivos[i][3], 2) + Math.pow(this.datos_objetivos[i][3], 2));
        }
    }
    ...
    public void caminar() {
        ...
        if (habilitado) {
            ...
            this.energia_consumida = distancia * this.propulsores[0].consumo_base * this.propulsores.length
                    + this.consola.consumo_base;
    ...
    public void correr() {
        ...
        if (habilitado) {
            ...
            int distancia = scanner.nextInt();
            this.energia_consumida = 2 * distancia * this.propulsores[0].consumo_base * this.propulsores.length
                    + this.consola.consumo_base;
            this.energia_armadura -= this.energia_consumida;
    ...
    public void propulsar() {
        ...
        if (habilitado) {
            ...
            int distancia = scanner.nextInt();
            this.energia_consumida = 3 * distancia * this.propulsores[0].consumo_base * this.propulsores.length
                    + this.consola.consumo_base;
            this.energia_armadura -= this.energia_consumida;
    ...
    public void destruirEnemigos() {
        ...
        for (int i = 0; i < this.datos_objetivos.length; i++) {
            if (this.hostilidades_objetivos[this.datos_objetivos[i][1]] == "hostil"
                    && this.distancias_objetivos[i] < 5000 && this.datos_objetivos[i][2] > 0) {
                hay_enemigos = true;
                break;
            }
        }
        if (!hay_enemigos) {
            ...
            for (int i = 0; i < this.datos_objetivos.length; i++) {
                if (this.hostilidades_objetivos[this.datos_objetivos[i][1]] == "hostil"
                        && this.distancias_objetivos[i] < 5000) {
                    while (this.datos_objetivos[i][2] > 0 && hay_energia) {
                        for (int j = 0; j < this.repulsores.length; j++) {
                            if (this.repulsores[j].estado == "sano") {
                                ...
                                this.datos_objetivos[i][2] -= (100 + this.repulsores[0].daño
                                        - 0.5 * this.distancias_objetivos[i]);
                            }
                        }
                        this.consola.mensajes("atacando", String.valueOf(i));
                        hay_energia = ((this.energia_armadura - this.energia_consumida)
                                / this.energia_armadura_maxima > 0.1);
                    }
                }
            }
            this.dañarRepulsores();
